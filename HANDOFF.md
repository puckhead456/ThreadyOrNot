...
## What is live (cache v16, 2026-09-16)

- **Crochet** (the original app): row/round + stitch counters, parts with make-counts, editable templates, checklists, pattern paste and in-app PDF import (pdf.js, on-device) through parser v2.1 (computed counts, multi-size, sections, notes, repeats), six themes, celebrations, guided tours + FAQ, export/import, proprietary LICENSE. **Live 3D diagram** in the stitch button (raw WebGL, `js/diagram.js` v1.1): count on release inside 12 px, swipe on the button spins the piece and never counts, drag follows the finger in button and viewer, yarn material + contact shadow. Not yet checked on real touch hardware or a mid-phone GPU.
...
## Tests (all green 2026-09-16)

| page | assertions | notes |
|---|---|---|
| `test/patterns.test.html` | 335 | crochet parser, synthetic |
| `test/patterns.fixtures.html` | 156 | real crochet PDFs in `tmp-pdf/` |
