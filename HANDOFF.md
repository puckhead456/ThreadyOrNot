...
## What is live (cache v18, 2026-09-17)

- **Crochet** (the original app): row/round + stitch counters, parts with make-counts, editable templates, checklists, pattern paste and in-app PDF import (pdf.js, on-device) through parser v2.2 (computed counts, multi-size, sections, notes, repeats; header forms `Name – colour:`, `Name in colour:`, `(4x)`, `Rnd 7+8:`, stranded wrapped counts, title-above-header; `PdfText` repairs letter-spaced PDFs per page), six themes, celebrations, guided tours + FAQ, export/import, proprietary LICENSE. **Live 3D diagram** in the stitch button (raw WebGL, `js/diagram.js` v1.1): count on release inside 12 px, swipe on the button spins the piece and never counts, drag follows the finger in button and viewer, yarn material + contact shadow. Not yet checked on real touch hardware or a mid-phone GPU.
...
|---|---|---|
| `test/patterns.test.html` | 367 | crochet parser, synthetic |
| `test/patterns.fixtures.html` | 186 | real crochet PDFs in `tmp-pdf/` |
