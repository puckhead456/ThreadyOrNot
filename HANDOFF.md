...
## What is live (cache v19, 2026-09-17)

- **Crochet** (the original app): row/round + stitch counters, parts with make-counts, editable templates (**templates carry pattern text** per part since v19), checklists, pattern paste and in-app PDF import (pdf.js, on-device; **drop the PDF straight into the New project sheet**: sections + checklist picker inline, synthetic "From this PDF" template card, "Also save as a template" toggle; `importPicker` in app.js is shared with the menu → Import pattern sheet) through parser v2.3 (computed counts, multi-size, sections, notes, repeats; header forms `Name – colour:`, `Name in colour:`, `(4x)`, `Rnd 7+8:`, stranded wrapped counts, title-above-header, `Name: (Make 2)`, slash names, duplicate headers, glossary guard; `PdfText` repairs letter-spaced PDFs per page and drops rotated / floating photo captions geometrically), six themes, celebrations, guided tours + FAQ, export/import, proprietary LICENSE. **Live 3D diagram** in the stitch button (raw WebGL, `js/diagram.js` v1.1): count on release inside 12 px, swipe on the button spins the piece and never counts, drag follows the finger in button and viewer, yarn material + contact shadow. Not yet checked on real touch hardware or a mid-phone GPU.
...
|---|---|---|
| `test/patterns.test.html` | 394 | crochet parser, synthetic |
| `test/patterns.fixtures.html` | 202 | real crochet PDFs in `tmp-pdf/` |
| `test/templates.test.html` | 73 | templates carry pattern text; createProject / templateFromProject / export round-trip |
...
- The service worker is cache-first, **including `js/*.js` loaded by the test pages**. Before testing ANY change: `navigator.serviceWorker.getRegistrations().then(rs=>Promise.all(rs.map(r=>r.unregister()))).then(()=>caches.keys()).then(ks=>Promise.all(ks.map(k=>caches.delete(k)))).then(()=>location.reload())`, then reload with a cache-busting query. Bump `CACHE_VERSION` in `sw.js` on every release (currently **v19**) and precache any new file.
...
- Deploy = push to `main` (single branch; the owner wants everything on main). Verify live files WITHOUT cache-busting query strings (GitHub's CDN returns 404 for `?x=` on fresh deploys); polling `sw.js` for the new `CACHE_VERSION` is the reliable check.
- Delegation pattern that has worked: write the contract (SPEC.md / CRAFTS.md), spawn one Opus agent per independent file group with strict file ownership, tell each to use its own Browser-pane tab (`tabs_create` + `tabId`) and to clear the service worker before testing, integrate, run every test page, verify at 375×812, bump the cache, deploy. Research-first for new crafts (one agent writes `docs/research/<craft>.md`, then builders implement Part B).
